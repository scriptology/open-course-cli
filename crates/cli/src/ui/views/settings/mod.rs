...
impl Section {
    fn label(&self, labels: &SettingsLabels) -> &'static str {
        match self {
            ...
            Section::Account => labels.section_account,
        }
    }
...
impl SettingsState {
    pub fn new() -> Self {
        ...
        Self {
            ...
            account: account::AccountState::default(),
        }
    }
...
fn build_body(state: &AppState, labels: ReportLabels, settings: &SettingsLabels) -> Text<'static> {
    ...
    if state.settings.section == Section::Account {
        return account::build_body(state, settings);
    }
...
fn footer_text(state: &AppState, width: usize) -> String {
    ...
    match state.settings.section {
        Section::Account => build_footer_wrapped(
            &[
                ("↑/↓", common.action),
                ("Enter", common.select),
                ("Esc", common.back),
...
pub fn draw(frame: &mut ratatui::Frame, area: ratatui::layout::Rect, state: &mut AppState) {
    ...
    if let Some(action) = state.settings.pending_reset {
        ...
        draw_confirmation(
            ...
            &settings
                .confirm_action
                .replace("{}", data::action_label(&action, &settings)),
            common.confirm_any_other,
        );
        return;
    }

    if state.settings.account.bind_dialog.is_some() {
        account::draw_bind_dialog(frame, area, state);
        return;
    }
...
pub async fn handle_key(state: &mut AppState, code: KeyCode) -> Result<()> {
    ...
    if state.settings.account.bind_dialog.is_some() {
        return account::handle_bind_dialog_key(state, code).await;
    }
    ...
    match code {
        ...
        KeyCode::Enter => {
            if state.settings.section == Section::Account {
                account::activate(state).await?;
            } else if state.settings.section == Section::Data {

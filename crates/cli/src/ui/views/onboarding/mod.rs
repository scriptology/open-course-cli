...
use open_course_config::{OpenCourseConfig, write_config};
...
pub(crate) async fn finish_onboarding(state: &mut AppState) -> Result<()> {
    match state.onboarding.mode {
        OnboardingMode::Initial => {
            ...
            let db_path = open_course_config::pair_db_path(&state.data_dir, &pair_id);
            let db = open_course_db::Database::connect(&db_path).await?;
            state.db = Arc::new(db);

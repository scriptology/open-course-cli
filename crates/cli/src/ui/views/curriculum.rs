...
use open_course_db::curriculum::{CEFR_LEVELS, Topic, difficulty_to_cefr};
...
impl CurriculumState {
    ...
    pub async fn load(&mut self, db: &open_course_db::Database) -> Result<()> {
        let (topics, progress_map) = open_course_service::curriculum::load_curriculum(db).await?;
...
pub async fn generate_curriculum(state: &mut AppState) -> Result<()> {
    ...
    tokio::spawn(async move {
        let result = open_course_service::curriculum::generate_curriculum(
            &config,
            &tx,
...
pub async fn reset_and_generate_curriculum(state: &mut AppState) -> Result<()> {
    open_course_service::curriculum::reset_curriculum_data(&state.db).await?;
...
async fn delete_selected_topic(state: &mut AppState) -> Result<()> {
    if let Some(topic) = state.curriculum.pending_delete.take() {
        open_course_service::curriculum::delete_topic(&state.db, &topic.id).await?;
...
pub async fn extend_curriculum(state: &mut AppState, count: usize) -> Result<()> {
    ...
    let tx = state.llm_tx.clone();
    tokio::spawn(async move {
        let result = open_course_service::curriculum::extend_curriculum(&db, &config, count).await;
